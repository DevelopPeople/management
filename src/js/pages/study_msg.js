define(['jquery', 'bootstrap', 'adminlte', 'pager', 'mine','mui'], function($, B, A, pager, mine,mui) {
    ...
    function getStudyList(page) {
        mine.showLoading();
        var url = urlBase + '/study/search/' + page + '/' + pageSize;
        mine.get(url).then(function(data) {
            ...
            if(data.errCode == 0) {
                ...
                mine.render("tpl/study_management.html", data).then(function(html) {
                    $('.task-list').html(html);
                    //	删除操作
...
//	})
    ...
    $('#addProduct').click(function() {
        ...
        if(memo != '' && pimg !== ''&& tit !='') {
            ...
            mui.alert('新增学习交流 主题、内容、上传logo、不能为空')
        }
    })

    function addStudy(dataJson) {
        mine.showLoading();
        var url = urlBase + '/study/add';
